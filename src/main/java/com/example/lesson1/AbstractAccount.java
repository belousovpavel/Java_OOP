...
//    private String number; номер счета, валюта
    ...
    public void take(BigDecimal decrease) {
        if(canTake(decrease)) {
            ...
            throw new IllegalArgumentException("Невозможно снять деньги");
        }
    }

    protected abstract boolean canTake(BigDecimal decrease);

    private void checkArgumentPositive(BigDecimal args) {
        if (args.compareTo(BigDecimal.ZERO) < 0) {
